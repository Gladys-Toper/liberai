...
let apiKey = ''
let scope: 'author' | 'admin' | 'agent' = 'author'
...
for (let i = 0; i < args.length; i++) {
  if (args[i] === '--api-key' && args[i + 1]) {
    ...
    scope = args[++i] as 'author' | 'admin' | 'agent'
...
if (!apiKey) {
  console.error('Usage: liberai-mcp --api-key lbr_live_xxx [--scope author|admin|agent] [--base-url https://liberai.com]')
  process.exit(1)
}
